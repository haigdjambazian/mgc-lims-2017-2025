...
@author: Alexander Mazur, alexander.mazur@gmail.com
    ...
    updated: 
        2018_08_31:
            - added AT_S and GC_S artifact fields to assign to LIMS
        2018_04_20
        - changed method of artifacts input output mapping - from active step to IOmap (input arts from Denaturation step output from Analysis QC
        2018_03_20 added hybrid option to:
         - read artifacts LUID from activeStep
         - read local artifacts LUID from local step        
        2018_02_04  
        - UDF fields write 
...
DEBUG = False
DEBUG_placement_file = "debug_test.csv"   # local file for testing
...
axiom_QC_udfs=['Sample Filename', 'QC', 'DQC', 'QC Call Rate', 'QC Het Rate', 'QC Computed Gender', 'Affymetrix Plate Barcode', 'Affymetrix Plate Peg Wellposition', 'Average Call Rate Passing Samples', '% of passing samples','AT_S','GC_S']
...
def get_samples_info_from_artifacts(sXML):
    samplesHash={} 
    pDOM = parseString( sXML )   
    for artifact in pDOM.getElementsByTagName("art:artifact"):
        artName = artifact.getElementsByTagName("name")[0].firstChild.data  # output artifact name
        ...
        sampleWell=artifact.getElementsByTagName("value")[0].firstChild.data 
        key=sampleLUID+"_"+sampleWell#+"_"+artLUID
        
        if key not in samplesHash:
            samplesHash[key]=artLUID+"xxx"+artName

    return samplesHash
def get_parent_info_from_artifacts(sXML):
    ...
    for artifact in pDOM.getElementsByTagName("art:artifact"):
        ...
        sampleLUID=artifact.getElementsByTagName("sample")[0].getAttribute("limsid")
        sampleWell=artifact.getElementsByTagName("value")[0].firstChild.data 
        key=artLUID #sampleLUID+"_"+sampleWell#+"_"+artLUID
        
        if key in iomap:
            iomap[key]=iomap[key]+"xxx"+artName
...
def create_hybrid_meta(localHash, activeHash):
    for key in activeHash:
        try:
            (activeArtID,activeSampleName)=activeHash[key].split("xxx")
            (localArtID,localSampleName)=localHash[key].split("xxx")
            
            sFileName=activeSampleName+"_"+activeArtID+".CEL"
            
            for line in axiomResultsTab:
                
                if line[0].upper() == sFileName.upper():
    
                    r=update_QC_flag(localArtID, line[1])
                    if DEBUG>2:
                        print(r)                
                    print (activeArtID+"=>"+localArtID+"\t"+line[1])
                    for i in range(1, len(line)):
                            
                        update_artifact_post(localArtID, axiom_QC_udfs[i],line[i], "")
        except:
            try:
                (localArtID,localSampleName)=localHash[key].split("xxx")
                print (localArtID+"\t"+localSampleName+"\t"+key+"\tcan't find artifact in the plate")
            except:
                print (key+"\t can't find artifact in the plate")

def create_parent_meta():
    for key in iomap:
        
        (localArtLUID,SampleName)=iomap[key].split("xxx")
            
        sFileName=SampleName+"_"+key+".CEL"
            
        for line in axiomResultsTab:
            if line[0].upper() == sFileName.upper():
                r=update_QC_flag(localArtLUID, line[1])
                if DEBUG>2:
                   print(r)                
                print (key+"=>"+localArtLUID+"\t"+line[1]+'\t'+SampleName)
                for i in range(1, len(line)):
                    #if DEBUG:
                    #print (localArtLUID, axiom_QC_udfs[i],line[i])    
                    update_artifact_post(localArtLUID, axiom_QC_udfs[i],line[i], "")
    '''
...
def create_io_map(stepURI_v2, sIO):
    ...
    for io in details.getElementsByTagName("input-output-map"):
        ...
        if (outputnode.getAttribute("type") == "Analyte") and (outputnode.getAttribute("output-generation-type")=="PerInput") :    # replicates, therefore multiple outputs per input
            if sIO=="input":
    #    INPUTS
                if inputartLUID not in localArtifactLUID:
                   localArtifactLUID.append( inputartLUID  )
            if sIO=="output":               
    #    OUTPUTS
                if inputartLUID not in localArtifactLUID:
                   localArtifactLUID.append( outputartLUID )



    return localArtifactLUID
...
def main():
    ...
    global api, user, psw,parentProcessLUIDs, activeProcessID, activeLUIDs,localContainers, localSamples_hash#,HOST, BASE_URI
    ...
    iomap=get_io_map(stepURI_v2)
    parentRIXML=prepare_artifacts_batch(iomap)

    artParentXML=retrieve_artifacts(parentRIXML)
    get_parent_info_from_artifacts(artParentXML)
    if DEBUG:
        print (iomap)
