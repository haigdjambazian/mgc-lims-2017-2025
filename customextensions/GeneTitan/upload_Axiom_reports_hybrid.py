...
def get_samples_info_from_artifacts(sXML):
    ...
    for artifact in pDOM.getElementsByTagName("art:artifact"):
        ...
        if sampleLUID not in samplesHash:
            samplesHash[sampleLUID]=artLUID+"xxx"+artName
...
def main():
    ...
    '''
        create Local Artifacts IO map
    '''
    localArtifactLUID=create_io_map(stepURI_v2,'output')
    #print (localArtifactURIs)
    localArtRIXML=prepare_artifacts_batch(localArtifactLUID)
    #print (localArtRIXML)
    if DEBUG:
       print (localArtRIXML)
    ...
    #print (localRetXML)
    localSamples_hash=get_samples_info_from_artifacts(localRetXML)
    if DEBUG:
        print (localSamples_hash)
...
#        print("local Container",localContainers)        
    ...
    '''
     Create list of parent processes to retirieve the Container Names
     activeStep="DNA Amplification McGill 1.1"
    ...
    '''        
    denaturationStepName='Denaturation and Hybridization McGill 1.0'
    ...
    for activeProcessID in activeLUIDs:
        ...
        print ("Denaturation process="+activeProcessURI)
         
        
        #map_artifacts_io( args.stepURI )
        #map_artifacts_io( activeProcessURI )
        '''
         containerSource="output" for Denaturation step
        '''
        activeArtLUIDs=create_io_map(activeProcessURI,containerSource)
        #print (activeArtLUIDs)
        sXML=prepare_artifacts_batch(activeArtLUIDs)
        if DEBUG>2:
            print(sXML)
        ...
        activeSamples_hash.update(get_samples_info_from_artifacts(retXML))

    #print (activeSamples_hash)
    if DEBUG:
        for key in localSamples_hash:
            print(key,localSamples_hash[key],activeSamples_hash[key])
    
        #create_meta(retXML)
    #print (axiomResultsTab)    
    create_hybrid_meta(localSamples_hash, activeSamples_hash)    
