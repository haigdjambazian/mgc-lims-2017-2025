...
NO_TAPE_new="/robot/GeneTitan/ProjectDATA/Axiom_Arrays/"
...
def get_meta_from_artifacts(artXML):
    ...
    for node in nodes.getElementsByTagName("art:artifact"):
        ...
        if artType == 'Analyte':
            ...
            projLUID=find_project(sampleLUID)
            if projLUID =="N/A":
...
def check_directory(meta_hash):
    ...
    for key in meta_hash:
        ...
        (activeContainerLUID,activeContainerName)=activeContainers[activeProcessID].split("xxx")
...
#        src_NO_TAPE=NO_TAPE_new+projectAcro+"_"+containerName+"_"+pProcessID+"/"+containerName+"_"+pProcessID
        ...
        #src_robot =NO_TAPE_new+projectAcro+"_"+projLUID+"/"+activeContainerName+"_"+pProcessID
        src_robot =NO_TAPE_new+activeContainerName+"_"+pProcessID
        #src_NO_TAPE=NO_TAPE_new+projectAcro+"_"+containerName+"_"+pProcessID+"/"+containerName+"_"+pProcessID
...
def main():
    ...
    if (args.stepURI) :
        ...
        denaturationStapName='Denaturation and Hybridization McGill 1.0'
        
        (denaturationProcessID,denaturationParentProcessID)=get_processID_by_processType(parentProcessLUIDs,denaturationStapName)
        denaturationProcessURI=BASE_URI+'steps/'+denaturationProcessID   
        print ('denaturation ProcessID='+denaturationProcessID)
        ...
        activeArtRIXML=prepare_artifacts_batch(activeArtURIs)
        if DEBUG:
            print (activeArtRIXML)
        activeArtifactsXML=retrieve_artifacts(activeArtRIXML)
        get_active_container_info(activeArtifactsXML)
        if DEBUG:
            print (activeContainers)

        #get_meta_from_artifacts(denArtifactsXML)        
        
        '''
        for parent step -"Denaturation...."
        '''
        denatArtURIs=create_io_map(denaturationProcessURI,'output')
        
        denArtRIXML=prepare_artifacts_batch(denatArtURIs)
        if DEBUG:
            print (denArtRIXML)
        denArtifactsXML=retrieve_artifacts(denArtRIXML)
        get_master_project(denArtifactsXML)
