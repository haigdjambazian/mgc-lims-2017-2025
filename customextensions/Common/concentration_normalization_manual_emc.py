...
def calculate_dilution(sSrcConcentration,sDestConcentration, sDestVolume):
    ...
    if (sSrcConcentration == 999):
        ...
        if (sTarget == "Quantity"):
            ...
            x=float(sDestConcentration)*(float(sDestVolume)-delta)/float(sSrcConcentration)

        w=float(sDestVolume)-x-delta    
    return x,w
...
def get_well_position_from_artifacts(sXML):    
    ...
    sEMCHeader='SampleName,Src,SrcWell,DstPlate,DstWell,VolSample,VolWater,VolCtrl,Sample LIMSID'
    
    print(sEMCHeader)
    proj="XXX"
    #sOUT="Sample Name,Container Name,Src ID,Src Coord,Dst ID,Dst Coord,Diluent Vol,Sample Vol,Sample LIMSID, Concentration_Org\n"
    sOUT=sEMCHeader+",Concentration_Org\n"
    i=1
    ...
    for child in root.findall('art:artifact',nss):
        ...
        container_name=get_container_name(dest_containerID)
...
#        print (Sample_Name)
        ...
        sOUT += Sample_Name+','+containerName+','+str(sSRCPosition)+','+container_name+','+well_position+','+str(x)+","+str(w)+','+str(iVolCtrl)+','+sample_ID+','+str(Concentration_org)+'\n'
        if w <0:
            w=0
        ...
        print(Sample_Name+','+containerName+','+str(sSRCPosition)+','+container_name+','+well_position+','+str(x)+","+str(w)+','+str(iVolCtrl)+','+sample_ID)
...
def main():
    if (stepURI_v2) :
        ...
        (sDestConcentration, sDestVolume, sControls, sThreshold, sConcentrationFrom, sTarget, iQuantity,iVolCtrl)=get_process_UDFs(processLuid)
