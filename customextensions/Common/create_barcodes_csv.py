...
def get_placed_containers(stepURI):
    ...
    for sc in scNodes:
        ...
        if scLUID not in outputContainers:
            outputContainers[scLUID]=scLUID+","+scName+","+scLUID
        #ss +=scLUID+","+scName+","+scLUID+"_Dst"+str(i)+"\n"
    i=1    #i +=1
    for key in sorted(outputContainers):
        ss +=outputContainers[key] +"_Dst"+str(i)+"\n"
        i +=1
...
def get_input_containers(retXML):
    ...
    for artifact in rDOM.getElementsByTagName("art:artifact"):
        ...
        if (artContainer not in inputContainers) and (len( artifact.getElementsByTagName( "control-type" )) ==0) :
            scName = get_container_name(artContainer)
            if artContainer not in inputContainers: 
                inputContainers[artContainer]=artContainer+","+scName+","+artContainer
            
            #ss +=artContainer+","+scName+","+artContainer+"_Src"+str(i)+"\n"
    i=1
    for key in sorted(inputContainers):
        ss +=inputContainers[key] +"_Src"+str(i)+"\n"
        i +=1
...
def main():
    global args, user,psw, containerSource
    args = setupArguments()
    ...
    if (containerSource =="output")  or (containerSource =="io"):
        ss= get_placed_containers(stepURI_v2)
    if (containerSource =="input") or (containerSource =="io"):
        map_io( stepURI_v2 )
