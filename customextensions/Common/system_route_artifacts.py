...
def main():
    ...
    global api,sutil,  allArtifacts,leftOverArtifacts, action, DEBUG,conditions,containersList
    ...
    #print(stepUDFs)
    if stepUDFs['Container Name']:
        get_containerID_by_name(stepUDFs['Container Name'],'')
        #print(containersList)
        ss=''
        if action!="route":
            for key in sorted(containersList):
                ss +=key +"\t"+containersList[key]+"\n"
            
            rr=update_process_udf(ProcessID, "Comments",ss, '')
            if len(containersList)==1:
                for key in containersList:
                    contID =key
                rr=update_process_udf(ProcessID, "Container ID",contID, '')
            else:
                rr=update_process_udf(ProcessID, "Container ID",'', '')
            print ("Done:\t"+str(len(containersList)) +" results found")    
