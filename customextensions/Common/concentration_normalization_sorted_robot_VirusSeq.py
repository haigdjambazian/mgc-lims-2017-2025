...
@author: Haig DJambazian
    update 2020_11_16
        - set minimum volume to 1uL
        - fix manual file attachement location in step
    update 2021_07_30
        - Add variable for sample volume cap to allow 15uL (from a previous maximum of 7.5uL) in VirusSeq/Nanopore workflow.

@author: Alexander Mazur, alexander.mazur@gmail.com
    update 2020_05_05:
        - low concentration fix str->float
    update 2019_10_10:
        - position for CSV manual file has been changed due to a kapatag file attachment
    update 2019_01_03:
...
def get_process_UDFs(ProcessID):
    global user,psw, hProjects,sDestConcentration, sDestVolume, sControls, sThreshold,sConcentrationFrom, sTarget, iQuantity, iVolCtrl, iSampleVolCap
...
#    sProjectName=getUDF(rDOM, 'AGCC Project')
    ...
    iSampleVolCap=getUDF(rDOM, 'VolCap')
    
    return sDestConcentration, sDestVolume, sControls, sThreshold,sConcentrationFrom, sTarget, iQuantity, iVolCtrl, iSampleVolCap    
...
def get_well_position_from_artifacts(sXML):    
    ...
    for child in root.findall('art:artifact',nss):
        ...
        (x,w,delta)=calculate_dilution(sConcentration,sDestConcentration, sDestVolume)
        if w <(float(sDestVolume)-float(iSampleVolCap)):
            x=float(iSampleVolCap)
            w=float(sDestVolume)-x
        if float(x) < 1:
            x=1
            w=float(sDestVolume)-x
...
#        print (Sample_Name)
        ...
        sOUT = sOUT + SrcID+','+str(sSRCPosition)+','+DstID+','+str(sRobotPosition)+','+str(new_w)+","+str(new_x)+','+well_position+','+sample_ID+','+str(Concentration_org)+'\n'        
...
#    print(sHeader)
    ...
    for child in root.findall('art:artifact',nss):
        ...
        if sample_ID not in Sample_Concentration:
            sConcentration=999
            containerNameSRC="N/A"
            containerIDSRC="N/A"
            containerNameSRC="N/A"
        else:
            (Concentration_org,src_position,SrcID, Sample_Name, containerIDSRC,containerNameSRC)=Sample_Concentration[sample_ID].split('__')
            sConcentration=Concentration_org
            sSRCPosition=get_phisical_position(src_position)
...
#        print (Sample_Name)
        ...
        sOUT = sOUT + dest_sample_name+','+containerNameSRC+'::'+containerIDSRC+','+src_position+','+sDestContName+'::'+dest_containerID+','+well_position+','+str(new_x)+","+str(new_w)+','+str(iVolCtrl)+','+sample_ID+','+str(Concentration_org)+'\n'
        ...
        if dest_index not in dest_sort_hash:
            ...
            dest_sort_hash[dest_index]=dest_sample_name+','+containerNameSRC+'::'+containerIDSRC+','+src_position+','+sDestContName+'::'+dest_containerID+','+well_position+','+str(new_x)+","+str(new_w)+','+str(iVolCtrl)+','+sample_ID
...
def print_manual_csv(sSortedOUT,sAttachemntPosition):
    if sSortedOUT:
        f_csv=open(sAttachemntPosition+"_for_manual.csv","w")
...
def main():
    if (stepURI_v2) :
        ...
        (sDestConcentration, sDestVolume, sControls, sThreshold, sConcentrationFrom, sTarget, iQuantity, iVolCtrl, iSampleVolCap)=get_process_UDFs(ProcessID)
    ...
    #        print (artif)
        ...
        #print("\n###################\n")
        #print(sSortedOUT)
        print_manual_csv(sSortedOUT,LUIDs[3])
