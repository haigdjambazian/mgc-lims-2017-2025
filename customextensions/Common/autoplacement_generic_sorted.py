...
'''
    ...
    Update:
        ...
        2018_04_30:
            - added my_auto_place() to create sequentially and in order the destination containers
        2018_07_19:
            - random CTRLs placement issue was solved by  replacing  <SampleName> to <SampleName>_<artifactLUID>
            - removed "templatefile" option from arguments
            - placementFile - all data for placement are stored in it.
...
def map_io( stepURI ):
    ...
    global details,ArtifactsLUID, sampleCounter, CTRLs, IO_hash
    iomap = {}
    ArtifactsLUID=[]
    sampleCounter={}
    CTRLs={}
...
#        if outputnode.getAttribute("type") == "ResultFile": 
            ...
            IO_hash[outputArtID]=inputartLUID
...
#        container_index, placement_value, control_name = resultsCSV[ c ]
    for c in range( len( placementFile)):
        ...
        for k in arts_hash:
            ...
            uqArtName=artName+"_"+IO_hash[k]
            #print (uqArtName,control_name.strip())
            if control_name.strip() == uqArtName:
                dest_containerURI = containerURIfromindex( container_index , args.containertype )
...
def get_all_well_position_from_artifacts(sXML):    
    ...
    for artifact in pDOM.getElementsByTagName("art:artifact"):
        ...
        sampleID_Position[artContainer+"_"+str(get_phisical_position(artLocation))]=artName +"xxx"+containerName+"xxx"+artLUID
    ...
    for k in sorted(containersCount):
        ...
        for iPos in range(1,97):
            ...
            if sKey in sorted(sampleID_Position):
                (artName ,containerName, artLUID)=sampleID_Position[sKey].split('xxx')
             ...
             #   f_out.write(str(iDestCont)+","+get_AlphaNumPosition(iDestSmpl)+","+sampleID_Position[sKey]+"\n")
                print (containerName+","+get_AlphaNumPosition(iDestSmpl)+","+artName+"_"+artLUID)
                placementFile.append([containerName,get_AlphaNumPosition(iDestSmpl),artName+"_"+artLUID])
                
                #f_out.write(containerName+","+get_AlphaNumPosition(iDestSmpl)+","+artName+"_"+artLUID+"\n")
