...
'''
    ...
    Update:
        ...
        2018_04_30:
            - updated to use  my_auto_place() with ResultsFile hash as a master against arts_hash[] 
            to create the destination containers in the sequential order. The robot CSV file will be created with correct
            samples position 
        2019_01_30:
            - added control NA24385 instead Axiom_gDNA
    ...
    for c in range( len( resultsCSV )):
        ...
        for k in arts_hash:
            ...
            if (control_name.strip() == artName) and (iCTRL==1):
                ...
                break
            if (control_name.strip() == artName) and (iCTRL==0) and (oArtLUID == k):
                dest_containerURI = containerURIfromindex( container_index , args.containertype )
                #outArtURI=arts_hash[control_name.strip()]
                output_art = outArtURI.split( "?" )[0]  # remove state
                if output_art not in  placementMap:
...
def get_left_over_container(stepURI):
    ...
    try: # there might not be an on the fly container
        conURI = parseString( api.GET( args.stepURI + "/placements") ).getElementsByTagName("selected-containers")[0].getElementsByTagName("container")[0].getAttribute("uri")
        print ("LeftOver Container :\t"+conURI)
        #print("DEBUG: Do not remove leftover container")
        print api.deleteObject( "", conURI )
    except:
        pass    
    ...
    for k in containersCount:
        ...
        for iPos in range(1,97):
            ...
            if sKey in sampleID_Position:
                (artName, artLUID)=sampleID_Position[sKey]
                ...
                print (str(iDestCont)+","+get_AlphaNumPosition(iDestSmpl)+","+artLUID)
                f_out.write(str(iDestCont)+","+get_AlphaNumPosition(iDestSmpl)+","+artLUID+"\n")
    ...
    for k in sorted(containersCount):
        for iPos in range(1,97):
            ...
            if sKey in sampleID_Position:
                ...
                zipSampleID_Position[str(newPositionCounter)+"_"+str(newContainer)]=(artName,artLUID)
                newPositionCounter +=1
...
#    ctrl_Apos=["A:1","D:0","G:11", "H:10"]
    ...
    before Jan. 2019
    '''        
    #ctrl_Ppos={1:"Axiom_gDNA103",44:"CEPH1463-02",87:"Negative_Control",80:"CEPH1463-02"}
    '''
    from Jan. 30 2019
    '''
    
    ctrl_Ppos={1:"NA24385",44:"CEPH1463-02",87:"Negative_Control",80:"CEPH1463-02"}
...
def main():
    ...
    global api, BASE_URI, user, psw,template_file
    ...
    #print (art_io_map)
    #outArtLUID=[oArtLUID for  oArtLUID ,iArtLUID  in art_io_map.items() if iArtLUID == '2-482952']
    #print (outArtLUID)
    ...
    if debug:
        ...
        for kk in sorted(arts_hash):
            print (kk, arts_hash[kk])
