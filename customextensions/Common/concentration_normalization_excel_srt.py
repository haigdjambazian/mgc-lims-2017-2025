...
@author: Alexander Mazur, alexander.mazur@gmail.com
    update 2018_12_04:
        - added "Mixed" mode handle tubes and plates in one run 
    update 2018_04_25:
        - print_plates_info_table added to create mapping table file 
        - added 'Negative_Control' and 'Axiom_gDNA103'
    update 2019_01_30:
        - added control NA24385 instead of Axiom_gDNA103
...
sDestConcentration=''
sDestVolume=''
Affy_Controls={'NA24385':"CtrlRack2",'CEPH1463-02':"CtrlRack2",'Negative_Control':"Trough1"}

#Affy_Controls={'Axiom_gDNA103':"CtrlRack1",'CEPH1463-02':"CtrlRack2",'Negative_Control':"Trough1"}
    ...
    if (sSrcConcentration == 999):
        ...
        if (sTarget == "Quantity"):
            try:
                ...
                print ("There are mixed tubes and plate, please use MIXED option")
                exit(111)
        else:
            try:
...
def get_concentration_from_artifacts(sXML):    
    ...
    for child in root.findall('art:artifact',nss):
        ...
        if (sConcentrationFrom == "Submitted Sample") and (isControlContainer is None): 
            Sample_Concentration[sample_ID]="xxx__"+well_position+"__"+containers_arr[containerID]+"__"+artifact_name        
        for sUDF in udf_child:
            if sUDF.attrib['name'] == 'Concentration':
                sConcentration=sUDF.text
                if sample_ID not in Sample_Concentration:
                    Sample_Concentration[sample_ID]=sConcentration+"__"+well_position+"__"+containers_arr[containerID]+"__"+artifact_name
...
def get_well_position_from_artifacts(sXML):    
    ...
    for child in root.findall('art:artifact',nss):
        ...
        if sample_ID not in Sample_Concentration:
            ...
            (Concentration_org,src_position,SrcID, Sample_Name)=Sample_Concentration[sample_ID].split('__')
...
#        print (Sample_Name)
        ...
        if dest_index not in dest_sort_hash:
            dest_sort_hash[dest_index]=SrcID+','+str(sSRCPosition)+','+DstID+','+str(sRobotPosition)+','+str(w)+","+str(x)+','+well_position+','+sample_ID
...
def get_well_position_from_mix_artifacts(sXML):    
    global dest_containers_arr,Affyreagent_counter,dest_sort_hash
...
def sort_human( input_hash ): 
  """ Sort the given list in the way that humans expect. 
  """ 
  convert = lambda text: int(text) if text.isdigit() else text 
  alphanum_key = lambda key: [ convert(c) for c in re.split('([0-9]+)', key) ] 
  list(input_hash.sort( key=alphanum_key ))    


def check_mixed_mode(Sample_Concentration):
    
    for key in Sample_Concentration: 
        (Concentration_org,src_position,SrcID, Sample_Name)=Sample_Concentration[key].split('__')
        if Concentration_org =="xxx":
            print ("There are missed concentration data...")
            exit(111)
...
def main():
    if (stepURI_v2) :
        ...
        Parse Tubes and Plates
        '''
        get_org_samples_array(lXML)
        samplesXML=prepare_samples_batch(sampleURIs)
        retSamplesXML=retrieve_samples(samplesXML)
        if (sConcentrationFrom == "Mixed"):             
            get_concentration_from_org_samples(retSamplesXML)
        ...
        check_mixed_mode(Sample_Concentration)
        
        get_well_position_from_mix_artifacts(dest_lXML)
