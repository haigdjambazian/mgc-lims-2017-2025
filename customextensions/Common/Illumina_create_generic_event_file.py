...
def get_artifact_back_path(global_IO_hash,artifactLUID, startProcessLUID, stopProcessLUID):
    ...
    for key in global_IO_hash:
        ...
        if (oArtifact==artifactLUID):
            #print  (startProcessLUID,processLUID,iArtifact,oArtifact,parentLUID, processName, iOrder)
        ...
        for k in IlluminaSeq_hash:
            ...
            for LibNormArt in LibNorm_hash:
                ...
                for liNormLUID, libProcessLUID in LibNormID.items():
                    get_artifact_back_path(global_IO_hash,iArtifactLUID, liNormLUID, ProcessID)
                    
                
                if len(aa_hash)==1:
                    LibNormArt=aa_hash[0]
                #if (DEBUG=='1') :
                #    print ("aa_hash=",aa_hash,iArtifactLUID,liNormLUID, ProcessID)
...
def main():
    ...
    if (iLibraryProcesses==1) or (iLibraryProcesses==0):
        ...
        global aa_hash
        aa_hash=[]
        get_artifact_back_path(global_IO_hash,'2-408622', '151-57084', ProcessID)
        if len(aa_hash)>0:
            print (aa_hash[0])
        else:
