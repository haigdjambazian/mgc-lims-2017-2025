...
'''
    ...
    Update:
        2020_02_13:
            - added support for 96->384 well plate  autoplacement
        2019_03_25:
...
def setup_arguments():
    ...
    Parser.add_option('-c', "--containertype", action='store', dest='containertype',default='96 well plate')    # destination container type <96 well plate|384 well plate>
    ...
    for c in range( len( resultsCSV )):
        ...
        for k in arts_hash:
            ...
            if (control_name.strip() == artName) and (iCTRL==1):
                ...
                break
            '''
            #if (control_name.strip() in artName) and (oArtLUID == k):
            if (control_name.strip() in artName) and (oArtLUID == k):
                if debug==True:
                ...
                #outArtURI=arts_hash[control_name.strip()]
                output_art = outArtURI.split( "?" )[0]  # remove state
                print(artName,output_art,outArtURI)
                if output_art not in  placementMap:
                    placementMap[ output_art ] = artifactLocation( dest_containerURI, placement_value )
                    print "contIndex=\t"+container_index+' = '+placement_value +" = "+control_name +" artName="+control_name.strip()+" desdURI="+dest_containerURI+" outArt="+output_art
                arts_hash.pop( k ) # remove from list of artifacts
        ...
        for iPos in range(1,97):
            ...
            if sKey in sorted(sampleID_Position):
                ...
                if containertype=="384 well plate":
                    (i384OhysPos, s384AlphaNumPos)=convert_96_to_384_AlphaNum(iDestSmpl)
                    sOUT +=containerName+","+s384AlphaNumPos+","+artName+"xxx"+artLUID+'xxx'+outArtLUID+"\n"                    
                else:
...
def get_384_AlphaNumPosition(iPhysPos):
    alpha=['P','A','B','C','D','E','F','G','H','I','J','K','L','M','N','O','P']
    iAlphaInt=iPhysPos // 16
    iAlphaRem=iPhysPos%16
    sSep=":"
    ...
    if iAlphaRem >0:
        ...
        s= alpha[iAlphaRem] +sSep+str(iAlphaInt)
    return s
def convert_96_to_384_AlphaNum(iPhysPos96):
    iCol=0
    for i in range(1,97):
        j384pos=(i-1)*2+1+iCol*16
        iCol=i//8
        if i==int(iPhysPos96):
            return j384pos,get_384_AlphaNumPosition(j384pos)
...
def main():
    ...
    global api, BASE_URI, user, psw,suffix,outputArtifact, placementCSV, containerNames_hash,containertype
    api = glsapiutil.glsapiutil2()
    api.setURI( args.stepURI )
    api.setup( args.username, args.password )
    stepInfo=glsapiutil.stepHelper()
    ...
    stepInfo.setAPIHandler(api)
    stepInfo.setStepURI(args.stepURI)
    containers=stepInfo.getSelectedContainers()
    print(containers)
    #exit()
