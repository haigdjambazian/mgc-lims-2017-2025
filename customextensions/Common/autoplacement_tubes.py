    ...
    for io in details.getElementsByTagName("input-output-map"):
        ...
        if (outputnode.getAttribute("type") == outputArtifact) :    # replicates, therefore multiple outputs per input
            ...
            if sampleLUID not in Samples_hash:
                Samples_hash[sampleLUID]=outputArtID
...
#    details = parse_xml( api.GET( subSampleURI ))
    ...
    for keys in details.getElementsByTagName("smp:sample"):
        ...
        carrierRack_hash[carrName+"_"+carrPosition] =(sampleLUID, sampleName)
    return
...
def sort_racks_by_submitted_oder(carrierRacks,carrierRack_hash):
    ...
    for key in carrierRack_hash:
        ...
        if iSampleLUID not in newSamples:
            (carrName,carrPosition)=key.split("_")
...
def main():
    ...
    global api, BASE_URI, user, psw,suffix,outputArtifact, debug
    ...
    for kk in sorted_carrierRacks:
        (cRackName,iOrder)=kk
        for jj in carrierRack_hash:
            (carrName,carrPosition)=jj.split("_")
            if carrName == cRackName:
                (sampleLUID, sampleName)=carrierRack_hash[jj]
                #print(carrName+"\t"+carrPosition+"\t"+sampleLUID)
