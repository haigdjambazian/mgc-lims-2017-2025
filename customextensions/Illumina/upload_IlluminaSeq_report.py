...
original name upload_NovaSeq_report.py has been changed to upload_IlluminaSeq_report.py
    updated 2020_02_03:
        - technology input -g mapping to  folder/step has been moved to protocols.txt
    updated 2019_02_15:
        - statistics for UDF updates added
    updated (Feb. 5, 2018):    
...
'''
valid input for -g parameter values:
    NovaSeq     
    HiSeqX      
    HiSeq4000   
    HiSeq2500   
    ...
    the mapping table for folders/steps from 2020_02_03 has been moved to the protocols.txt     
'''
...
def setup_arguments():
    ...
    Parser.add_option('-g', "--technology", action='store', dest='technology') # HiSeqX, NovaSeq, HiSeq4000,HiSeq2500, iSeq, MiSeq
    ...
    for line in illuminaResultsTab:
        ...
        if line[1] in uploads_hash:
            ...
            if (line[1] !="UDF") and (line[1] !="UDF_PFURI"):
                ...
                if (udfCheckName not in udfs_hash) or (udfs_hash[udfCheckName]!="Complete"):
                    ...
                    if len(lXML)>0:
                        ...
                        r = api.POST(lXML,sURI)
                        try: 
                            udfStats_hash['File_attached']=udfStats_hash['File_attached']+1
                        except:
                            udfStats_hash['File_attached']=1
                        print (r)
                else:
            ...
            if (line[1] =="UDF") and (line[2][0:3]!='24-') :
                ...
                udfValue=line[4]
                udfType="String"
                udfValueDecoded=udfValue.decode('ascii','')
                rr=update_artifact_post(artID, udfName,udfValueDecoded, udfType)
                try:
                    udfStats_hash['UDF_sample']=udfStats_hash['UDF_sample']+1
                except:
                    udfStats_hash['UDF_sample']=1
                #print (rr)
            if (line[1] =="UDF") and (line[2][0:3]=='24-') :
                #bPublish="true"
                processID=line[2]
                ...
                try:
                    udfStats_hash['UDF_process']=udfStats_hash['UDF_process']+1
                except:
                    udfStats_hash['UDF_process']=1
                #print (rr)
                ...
                if udfCheckName not in udfs_hash:
                    ...
                    if len(lXML)>0:
                        ...
                        if len(nodes) > 0:
                            ...
                            dlURI= new_HOSTNAME+"/lablink/secure/DownloadFile.do?id="+fLUID.split("-")[1]
                    ...
                    rr=update_process_udf(processID, udfName,udfValue, udfType)
                    try:
                        udfStats_hash['UDF_RFURI']=udfStats_hash['UDF_RFURI']+1
                    except:
                        udfStats_hash['UDF_RFURI']=1
                    #print (rr)
...
def read_full_config_file(sFileName):
    
    config = ConfigParser.RawConfigParser()
    config.read(sFileName)
    LibPooling=config.get(technology,'LibPooling')
    LibNorm=config.get(technology,'LibNorm')
    ClusterGen=config.get(technology,'ClusterGen')
    Sequencing=config.get(technology,'Sequencing')
    StorageFolder=config.get(technology,'StorageFolder')
    StorageSuffix=config.get(technology,'StorageSuffix')
    if DEBUG:
        print (LibPooling,LibNorm,ClusterGen)
    return LibPooling,LibNorm,ClusterGen,Sequencing,StorageFolder,StorageSuffix
...
def main():
    ...
    global api, user, psw,inputFIleLUID, technology, fileTemplate,udfStats_hash #,HOST, BASE_URI
    ...
    ppUDFValue={}
    udfs_hash={}
    LibPooling,LibNorm,ClusterGen,Sequencing,StorageFolder,StorageSuffix=read_full_config_file(script_dir+"/protocols.txt")
    get_parent_process(ProcessID,'')
    
    sClusterStepName=ClusterGen
    #print(sClusterStepName)
    #exit()
    ppUDFValue=get_rb_parentProcess_UDF(parentProcessLUIDs,sClusterStepName,"Start Date")
    sStartDate=ppUDFValue['Start Date']
    (sClusterLUID, currentLUID)=get_processID_by_processType(parentProcessLUIDs,sClusterStepName)
    
    
    sContName= get_container_info_from_step(sClusterLUID)
    #print (sContName)
    ...
    if args.template:
        ...
        if inputFIleLUID:
            attachedFile=get_isfile_attached(inputFIleLUID)
            if attachedFile:
                bFileAttached=1

        if bFileAttached==1:
            get_illumina_reports_from_file('inputFIleLUID')
            post_results_data_2_lims()
        


        sDir="/lb/robot/research/processing/" + StorageFolder+ "/" + sStartDate.split("-")[0] + "/" + "*" + sContName + "*" + "-" + StorageSuffix
        #print (sDir)
        try:
            runFolderPath = glob.glob(sDir)[0]
            sBaseDir=runFolderPath.split('/')[-1]
            sUDF_file=runFolderPath+"/"+sBaseDir+"-run.db_upload.udfs.txt"
            sUDFinit_file=runFolderPath+"/"+sBaseDir+"-run.db_upload_init.udfs.txt"
        except:
            print ("Cant find folder "+sDir)  
            sys.exit(111)  
        #(sRootDir,technology,sYear,sRunID,sBaseDir,sFullPath,sUDF_file,sUDFinit_file)=get_base_dir_meta(udfs_hash)
        #print(runFolderPath, sBaseDir,sUDF_file,sUDFinit_file)
        ...
        #print(attachedFile, bFileAttached)
        ...
        #(sRootDir,technology,sYear,sRunID,sBaseDir,sFullPath,sUDF_file,sUDFinit_file)=get_base_dir_meta(udfs_hash)
        #print (sRootDir,technology,sYear,sRunID,sBaseDir,sFullPath,sUDF_file,sUDFinit_file)
        bInit=0
        bFull=0
        if os.path.isfile(sUDFinit_file):
            bInit=1
        if os.path.isfile(sUDF_file):
            bFull=1
    
        print(bInit,bFull)
        #print (sUDFinit_file,sUDF_file )
        #exit()
        ...
        if (bInit==1):
            get_illumina_reports_from_file('sUDFinit_file')
            post_results_data_2_lims()
        if (bFull==1):
            get_illumina_reports_from_file('sUDF_file')
            post_results_data_2_lims()
       
        if (bInit==1) :
            update_process_udf(ProcessID, "Run Processing Status","In progress", "String")
        if (bFull==1):
            update_process_udf(ProcessID, "Run Processing Status","Complete", "String")    
    
        for key in udfStats_hash:
            print(key+'=\t'+str(udfStats_hash[key]))
