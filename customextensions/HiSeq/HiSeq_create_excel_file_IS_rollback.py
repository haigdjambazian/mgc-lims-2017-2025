...
def get_rollback_libProtocol_artifact():
    ...
    for key in activeProcesses_hash:
        ...
        for node in rDOM.getElementsByTagName("input-output-map"):
            ...
            if sampleLUID not in SamplesLib_hash:
                SamplesLib_hash[sampleLUID]=outputLUID+"xxx"+processName
...
def create_rollback_tab_del_output_file(sXML):
    ...
    for node in nodes:
        ...
        lib_Protocol="N/A"
        smplTag="N/A"
        lib_art = SamplesLib_hash[sampleLUID].split("xxx")[0]
        if DEBUG:
           print("sampleLUID",sampleLUID,SamplesLib_hash[sampleLUID])
        if (lib_art != "N/A"):
            lib_artifact_luid=lib_art
            lib_Protocol=SamplesLib_hash[sampleLUID].split("xxx")[1] 
            smplTag=get_lib_artifact_UDF(lib_art, "Sample Tag")
            if DEBUG:
...
def main():
    ...
    if (stepURI_v2) :
        ...
        get_projects_list()
        global sLibraryKit, sBCLMode, Reference, ProcessID,ppUDFValue,parentIDs,allParentsIDs,artifactsName,parentProcessLUIDs
        ppUDFValue={}
        parentIDs = []
        allParentsIDs={}
        artifactsName={}
        ...
        #exit()
        get_rollback_libProtocol_artifact()
#        for k in SamplesLib_hash:
#            print (k, SamplesLib_hash[k])
        #exit()
        if DEBUG:        
...
#        get_parentProcess_UDF(allParentsIDs,"Cluster Generation (HiSeq X) 1.0 McGill 1.0","Flowcell Lot")
        ...
        outFile=create_rollback_tab_del_output_file(zXML)
        print (outFile) # print the output as an attachment 
        write_event_file(outFile, sEventPath)
        
        #sSettingsOut= create_process_settings_file()
        #write_file2(sSettingsOut)        
