...
def get_map_io_by_process(processLuid, artifactType):
    ## get the process XML
    map_io={}
    pURI = BASE_URI + "processes/" + processLuid
    #print(pURI)
...
def get_map_io_by_process_generic(processLuid, artifactType, outputgenerationType,keyIO):
    ## get the process XML
    map_io={}
    pURI = BASE_URI + "processes/" + processLuid
    #print(pURI)
...
def get_io_from_stepName(sStepName):
    ...
    for key in parentProcessLUIDs:
        if (sStepName == parentProcessLUIDs[key].split(",")[1] ):
            ...
            map_io=get_map_io_by_process(key, "Analyte")
    return map_io


def get_io_from_stepS(sStepName, artifactTYpe, ogType,keyIO):
    ...
    for key in parentProcessLUIDs:
        if (sStepName == parentProcessLUIDs[key].split(",")[1] ):
            ...
            map_io=get_map_io_by_process_generic(key, artifactTYpe,ogType,keyIO)
...
def create_rollback_tab_del_output_file(sXML):
    global projLUID,projName, sampleLUID,sampleName
    pDOM = parseString( sXML)
    ...
    for node in nodes:
        ...
        lib_Protocol="N/A"
        smplTag="N/A"
        for nLib in SamplesLib_hash:
            (libSMPLID,outArtID)=nLib.split("_")
...
#                   print("sampleLUID",sampleLUID,SamplesLib_hash[nLib])
                if (lib_art != "N/A"):
                    lib_artifact_luid=lib_art
                    lib_Protocol=SamplesLib_hash[nLib].split("xxx")[1] 
                    smplTag=get_lib_artifact_UDF(lib_art, "Sample Tag")
                    if DEBUG:
                        print("Lib_art",lib_art,lib_artifact_luid,lib_Protocol,smplTag)       
...
def main():
    ...
    if (stepURI_v2) :
        ...
        if sResult !='N/A':
            isLibBatch=1
            map_io_LibBatch=get_io_from_stepS("Library Batch","Analyte","PerInput","input")
...
#        get_artifacts_from_stepName("Library Normalization (HiSeq X) 1.0 McGill 1.4")
        global map_io_LibNorm
        map_io_LibNorm=get_io_from_stepName("Library Normalization (HiSeq X) 1.0 McGill 1.4")
        
        global Lib_hash, Norm_hash
        Lib_hash={}
        Norm_hash={}
        if DEBUG:
        ...
        for key in map_io_LibNorm:
            (Lib_artifactLUID, arttifactName,containerID,reagentLabel,libProcessLUID, libProcessName, sampleLUID,sudfValue)=get_artifact_info_LIB(key, "Sample Tag")
            if Lib_artifactLUID not in Lib_hash:
                ...
                if DEBUG:
                    print (Lib_artifactLUID, arttifactName,containerID,reagentLabel,libProcessLUID, libProcessName, sampleLUID,sudfValue)
        
        if DEBUG:
            print ("\n############\n")
        for key in map_io_LibNorm:
            artOut=map_io_LibNorm[key]
            (Norm_artifactLUID, arttifactName,containerID,reagentLabel,libProcessLUID, libProcessName, sampleLUID,sudfValue)=get_artifact_info_LIB(artOut, "Sample Tag")
